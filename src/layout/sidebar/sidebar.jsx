...
const Sidebar = ({ links = [], activeItem, setActiveItem }) => {
  ...
  const handleItemClick = (link, index) => {
    ...
    if (link.type !== null) {
      const selector = link.type ? `.${link.type}${link.id}` : null;
      if (selector) {
  ...
  const renderLinks = () => {
    // Process links to identify wrappers and their content
    ...
    links.forEach((link, index) => {
      ...
      if (link.wrapper) {
        ...
        // Start a new group
        currentWrapper = { ...link, originalIndex: index };
        currentGroup = [];
      } else {
        // Add item to current group
        currentGroup.push({ ...link, originalIndex: index });
      }
    });
    ...
    return processedGroups.map((group, groupIndex) => {
      // Render wrapper label if it exists
      const wrapperElement = group.wrapper && (
        <span key={`wrapper-${group.wrapper.originalIndex}`} className="wrapper-label">
          {group.wrapper.label}
        </span>
      );
      ...
      // Render items for each type
      const typeElements = Object.entries(itemsByType).map(([type, typeItems]) => {
        // Single items don't need a dropdown
        if (typeItems.length === 1) {
          const item = typeItems[0];
          return (
            <li
              key={`${item.to}-single`}
              className={`nav-item ${activeItem === item.originalIndex ? 'active' : ''}`}
        ...
        return (
          <div key={`${type}-${groupIndex}`} className="dropdown-group">
            <div
              className="dropdown-header nav-item"
              onClick={(e) => toggleGroup(`${type}-${groupIndex}`, e)}
            >
              <ComponentIcon />
              <span className="group-title">{type.replace('DatoCms', '').replace(/([A-Z])/g, ' $1').trim()} </span>
              {expandedGroups[`${type}-${groupIndex}`] ? <span className="items-counter"><ChevronUp size={16} /></span> : <span className="items-counter">{typeItems.length}</span>}
            </div>
            {expandedGroups[`${type}-${groupIndex}`] && (
              <ul className="dropdown-items">
                {typeItems.map((item) => (
                  <li
                    key={`${item.to}-${item.id}`}
                    className={`nav-item nested ${activeItem === item.originalIndex ? 'active' : ''}`}
                    onClick={() => handleItemClick(item, item.originalIndex)}
                  >
                    <ComponentIcon />
                    {item.label}
